...
export async function criarUsuario(req,res) {
    ...
    if(!usuario.login || !usuario.senha || !usuario.funcao){
        res.status(400).json({message:'Login, senha e função são obrigratórios'})
...
export async function logarUsuario(req, res) {
    console.log('UusuarioController :: logarUusuario');
    const {login, senha} = req.body;

    if(!login || !senha){
        res.status(400).json({message:'Usuario e senha são obeso'});
    }else{
        try {
